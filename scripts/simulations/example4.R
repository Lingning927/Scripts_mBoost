...
# Example 4: one replicate of the linear-logistic feature redundancy
# experiment. The manuscript summarizes 100 repeated runs.
n <- 200
p <- 100
real_p <- 20
...
gene_data <- function(n, p, real_p) {
  ...
  prob <- 1 / (1 + exp(-X %*% true_coef - rnorm(n, 0, 4)))
...
# Select nonzero Lasso coefficients as candidate features.
...
# Feature-validity test for Lasso-selected variables based on overlap
# between two random data splits.
...
# Compare ordinary logistic regression with Lasso logistic regression.
